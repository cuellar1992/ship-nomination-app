...
router.get("/with-email", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching samplers with email" });
  }
});
...
router.get("/without-email", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching samplers without email" });
  }
});
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        emailCoverage:
          totalSamplers > 0
            ? Math.round((samplersWithEmail / totalSamplers) * 100)
            : 0,
      },
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, error: "Error fetching sampler stats" });
  }
});
