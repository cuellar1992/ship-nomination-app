...
shipNominationSchema.index({ 'client.name': 1 });
...
shipNominationSchema.virtual('displayName').get(function() {
    const etbDate = this.etb ? new Date(this.etb).toLocaleDateString('en-GB') : 'TBD';
...
shipNominationSchema.virtual('fullDisplayName').get(function() {
    const etbDate = this.etb ? new Date(this.etb).toLocaleDateString('en-GB') : 'TBD';
    const etbTime = this.etb ? new Date(this.etb).toLocaleTimeString('en-GB', { 
        hour: '2-digit', 
        minute: '2-digit' 
    }) : '';
...
shipNominationSchema.pre('save', async function(next) {
    try {
        ...
        if (dateErrors.length > 0) {
            const error = new Error(`Date validation failed: ${dateErrors.join(', ')}`);
            return next(error);
        }
        ...
        if (duplicateErrors.length > 0) {
            const error = new Error(`Duplicate validation failed: ${duplicateErrors.join(', ')}`);
            return next(error);
        }
...
shipNominationSchema.statics.checkAmspecExists = function(amspecRef, excludeId = null) {
...
shipNominationSchema.statics.checkClientRefExists = function(clientRef, excludeId = null) {
...
shipNominationSchema.statics.findWithFilters = function(filters = {}) {
    ...
    if (filters.client) {
        query['client.name'] = filters.client;
    }
